...
async function getAllVacationsAsync() {
  ...
  FROM vacations v left outer join selectedvacationbyuser f 
...
async function AddFollowVacationAsync(followedVacationAndUserObject) {
  ...
  const sql = `INSERT INTO selectedvacationbyuser (userId, vacationId) 
              VALUES ('${userId[0].userId}', '${vacationId}')`;
  await dal.executeAsync(sql);
}

async function deleteFollowedVacationAsync(id) {
  const sql = `DELETE FROM selectedvacationbyuser WHERE vacationId= ${id}`;
  await dal.executeAsync(sql);
}
